...
try{
    ...
    $sheet->setCellValue('D'.$rowTitle, "Actividad");
    $sheet->setCellValue('E'.$rowTitle, "# Preguntas");
    ...
    if ($resultExcel->num_rows > 0) {
       ...
       while($data = $resultExcel->fetch_assoc()){
            ...
            $sheet->setCellValue('C'.$row , $data["unidadTematica"]);
            $sheet->setCellValue('D'.$row , $data["autoevaluacion"]);
            $sheet->setCellValue('E'.$row , $data["preguntas"]);
            $row++;
       }
    }
    $stmtExcel->close();
    ...
    $nameReport = getNameFile($hashReport, "2Auto_SIGAUT_NoPregunta" . $nameProgram, "xlsx");
